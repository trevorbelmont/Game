...
class Heroe {
  ...
  void kinematics() {
    ...
    for (int i = 0; i <nObs; i++) {
      if (abs(x - obx[i]) <= 300 || abs(x - obx[i] + obtx[i]) <= 300 || x >= obx[i] && x <= obx[i] + obtx[i]) {
        if (y+tamY >= oby[i] && y+tamY <= oby[i]+obty[i] && x >= obx[i] && x <= obx[i]+obtx[i] && vy >0) {
          y = oby[i]-tamY;
          noSolo = true;
          inAir = false;
          onObs = true;
        ...
        if (x-tamX/2 <= obx[i] + obtx[i] && x-tamX/2 >= obx[i] && y+tamY > oby[i] && y+tamY < oby[i]+obty[i]) {
          leftBlock = true;
        }
        else {
          leftBlock = false;
       ...
       if (x+tamX/2 >= obx[i] && x+tamX/2 <= obx[i]+ obtx[i] && y+tamY > oby[i] && y+tamY < oby[i]+obty[i]) {
          rightBlock = true;
        }
        else {
          rightBlock = false;
