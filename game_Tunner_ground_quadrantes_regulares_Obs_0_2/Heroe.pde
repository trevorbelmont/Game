...
class Heroe {
  ...
  void kinematics() {
    ...
    for (int i = 0; i <nObs; i++) {
      ...
      if (abs(obx[i] - x) <= 300 || abs( x - (obx[i] + obtx[i])) <= 300 ) {
        //println(leftBlock +" " + rightBlock);
     ...
     /*     if(acimaObs == true && inAir == true){
            acimaObs = false;
          }*/
        
        if ( x + tamX/2 >= obx[i] && x - tamX/2 <= obx[i] + obtx[i]) {
      //    if (y+tamY >= oby[i] && y <= oby[i]+obty[i] && vy >0) { //&& x >= obx[i] && x <= obx[i]+obtx[i]
          if(overObs == true && overObs2 == false || acimaObs == true){
            acimaObs = true;
            y = oby[i]-tamY;
            noSolo = true;
            inAir = false;
