 ...
 */
/*
float obx[] = new float [100];
 float oby[] = new float [100];
 float obtx[] = new float [100];
 float obty[] = new float [100];
 */
int nQuadrantes = 20;
...
int tela = int(venom.x-width*0.75);
  ...
  for (int i = beginQuad; i<endQuad; i++) {
    ...
    strokeWeight(10);
    beginShape();
    vertex((i-1)*tamQuadrante, ground[i-1]);
   // bezierVertex((i-1)*tamQuadrante, ground[i-1], ((i-1)*tamQuadrante+i*tamQuadrante)/2, ((ground[i-1] +ground[i])/tamQuadrante)*((tamQuadrante/2) *(tamQuadrante/2)) , i*tamQuadrante, ground[i]);
    bezierVertex((i-1)*tamQuadrante, ground[i-1], (i-1)*tamQuadrante + tamQuadrante/2, (((ground[i-1] -ground[i])/tamQuadrante)*((tamQuadrante/2) *(tamQuadrante/2)))*0.1 , i*tamQuadrante, ground[i]);
    //bezierVertex((i-1)*tamQuadrante, ground[i-1], ((i-1)*tamQuadrante+i*tamQuadrante)/2, (ground[i-1] +ground[i])/2, i*tamQuadrante, ground[i]);
    endShape();
  }
