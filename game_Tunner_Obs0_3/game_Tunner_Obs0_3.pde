//CENTRALIZAR AS IMAGENS DA DATA
...
void setup() {
  ...
  size(screen.width,screen.width,P3D);
  background(0);
  fill(0);
...
void mousePressed() {
  if (mouseButton == LEFT) {
    obx[thisObs] = mouseX + cameraX;
    oby[thisObs] = mouseY - cameraY;
  }
}
void mouseReleased() {
  if (mouseButton == LEFT) {
    obtx[thisObs] = (mouseX+cameraX) - obx[thisObs];
    obty[thisObs] = (mouseY-cameraY) -oby[thisObs];  
    if (thisObs < nObs) {
      thisObs++;
    }
  }
...
void draw() {
   if (key == BACKSPACE) {
    println(rendimento + " ----------- " + millis() + " = " + rendimento/( millis()/1000) + " Ciclos por Segundos") ;
    exit();
  }
  else {
    rendimento++;
...
void debug(){
 ...
 stroke(155);
 textFont(fonte,15);
 text( "pulando=" + str(venom.pulando) + " | inAir=" + str(venom.inAir) + " | jumpTrava=" + str(venom.jumpTrava) + " | noSolo=" + str(venom.noSolo) + "\n" 
 + " pulou=" + str(venom.pulou) + " | acimaObs=" + str(venom.acimaObs[0]), 15,15);
 popMatrix();
